# =====================================================
# Función que carga y valida el dataset South German Credit
# =====================================================

read_SouthGermanCredit <- function(path = "GermanCredit/SouthGermanCredit.asc") {
  ...
  levels(dat$credit_history) <- c("delay in paying off in the past",
                                  "critical account/other credits elsewhere",
                                  "no credits taken/all credits paid back duly",
                                  "existing credits paid back duly till now",
                                  "all credits at this bank paid back duly")
  levels(dat$purpose) <- c("others", "car (new)", "car (used)",
  ...
  levels(dat$savings) <- c("unknown/no savings account",
                           ...
                           "500 <= ... < 1000 DM", 
                           "... >= 1000 DM")
  levels(dat$employment_duration) <- c("unemployed", "< 1 yr", 
                                       "1 <= ... < 4 yrs",
                                       "4 <= ... < 7 yrs", 
                                       ">= 7 yrs")
  dat$installment_rate <- ordered(dat$installment_rate)
  levels(dat$installment_rate) <- c(">= 35", "25 <= ... < 35",
                                    "20 <= ... < 25", "< 20")
  levels(dat$other_debtors) <- c("none", "co-applicant", "guarantor")
  levels(dat$personal_status_sex) <- c("male : divorced/separated",
                                       "female : non-single or male : single",
                                       "male : married/widowed",
                                       "female : single")
  dat$present_residence <- ordered(dat$present_residence)
  levels(dat$present_residence) <- c("< 1 yr", "1 <= ... < 4 yrs", 
                                     "4 <= ... < 7 yrs", ">= 7 yrs")
  levels(dat$property) <- c("unknown / no property",
                            "car or other",
                            "building soc. savings agr./life insurance", 
                            "real estate")
  levels(dat$other_installment_plans) <- c("bank", "stores", "none")
  ...
  levels(dat$job) <- c("unemployed/unskilled - non-resident",
                       "unskilled - resident",
  ...
  # ---- tablas para validación ----
  tabs <- list(
    status = round(100*prop.table(xtabs(~status+credit_risk, dat),2),2),
  ...
  return(list(data = dat, tabs = tabs, tabwhich = tabwhich))
}

# =====================================================
# Ejemplo de uso
# =====================================================
# source("read_SouthGermanCredit.R")
# res <- read_SouthGermanCredit()
# dat <- res$data
# tabs <- res$tabs
# tabwhich <- res$tabwhich
